...
class Trainer(object):
    ...
    def train(self):
        ...
        # def objective(trial):
        #     solver = trial.suggest_categorical('solver', ['lsqr']) # svd 오류남
        #
        #     if solver == 'lsqr':
        #         shrinkage = trial.suggest_categorical('shrinkage', [None, 'auto', 'float'])
        #         if shrinkage == 'float':
        #             shrinkage = trial.suggest_float('shrinkage_value', 0, 1)
        #
        #     lda = LinearDiscriminantAnalysis(solver=solver, shrinkage=shrinkage if solver == 'lsqr' else None)
        #     lda.fit(train_df, train_y)
        #
        #     preds = lda.predict(test_df)
        ...
        lda = LinearDiscriminantAnalysis(solver='lsqr', shrinkage=0.0190958490425972)
        lda.fit(train_df, train_y)
